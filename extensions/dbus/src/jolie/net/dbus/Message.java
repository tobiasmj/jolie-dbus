...
public class Message
{
   ...
   protected void appendBytes(byte[] buf) 
   {
      ...
      if (preallocated > 0) {
         if (paofs+buf.length > pabuf.length)
            throw new ArrayIndexOutOfBoundsException(MessageFormat.format(("Array index out of bounds, paofs={0}, pabuf.length={1}, buf.length={2}."), new Object[] { paofs, pabuf.length, buf.length }));
   ...
   private int appendone(byte[] sigb, int sigofs, Object data) throws DBusException
   {
      try {
         ...
         throw new MarshallingException(MessageFormat.format(("Trying to marshall to unconvertable type (from {0} to {1})."), new Object[] { data.getClass().getName(), sigb[sigofs] }));
      }
   }
   /**
...
//            rv = new UInt16((int) demarshallint(buf, ofs[1], 2));
 ...
 //           rv = new UInt64(top, bottom);
         ...
         case ArgumentType.ARRAY:
            ...
            switch (sigb[ofs[0]]) {
               ...
               case ArgumentType.DICT_ENTRY1:
                  ...
                  rv = null;
                  // TODO rv = new DBusMap<Object, Object>(entries.toArray(new Object[0][]));
                  break;
               default:
                  if (0 == size) {
                     // advance the type parser even on 0-size arrays.
         ...
         case ArgumentType.OBJECT_PATH:
            length = (int) demarshallint(buf, ofs[1], 4);
            ofs[1] += 4;
            // TODO rv = new ObjectPath(getSource(), new String(buf, ofs[1], length));
            rv = null;
            ofs[1] += length + 1;
            break;
         case ArgumentType.SIGNATURE:
   ...
   { 
      if(headers.get(HeaderField.ERROR_NAME) != null){
